...
if __name__ == '__main__':
    ...
    fine_tune_steps = [100, 300, 500, 800]

    # test
    all_rew = []
    for fine_tune_step in fine_tune_steps:
        rew = test_kitchen(env_name, demo_file, high_level_net, sub_skills, fine_tune_step)
        all_rew.append(rew)
        print('test_skill_num=', args.test_skill_num, ' suite=', args.test_suite, ' finetune_step=', fine_tune_step, ' rew=', rew)

    all_rew = np.array(all_rew).reshape(-1,)
    np.save('skill_'+str(args.test_skill_num)+'_suite_'+args.test_suite+'_all_rews.npy', all_rew.sort()[-5:])
